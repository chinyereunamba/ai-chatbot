...
export default function Home() {
  ...
  }, [history]); // Runs whenever new messages arrive
      ...
      while (true) {
        ...
        setHistory((prev) => {
          ...
          if (lastMessage?.role === "bot") {
            return [...prev.slice(0, -1), { role: "bot", message: aiResponse }];
          } else {
  ...
  function handleKeyDown(event: KeyboardEvent) {
    if (event.key === "Enter" && !event.shiftKey) {
      // Prevents new lines in textarea
  ...
  return (
    <main className="flex divide-x-2 h-screen">
      <aside className="max-w-[350px] w-full p-4">ASIDE</aside>
